...
const likePost = (req, res) => {
        ...
        Post.findByIdAndUpdate({ _id: req.body.postId }, { $push: { likes: like }, $inc: { totalLike: 1 } })
            .exec((err, result) => {
                if (err || !result) {
                    return res.status(400).json({
                        error: errorHandler.getErrorMessage(err)
...
const checkLikePost = (req, res) => {
    ...
    Post.find({ _id: postId }, { likes: { $elemMatch: { likeBy: userId } } })
        .exec((err, result) => {
...
const unLikePost = (req, res) => {
    var like = {};
    like.likeBy = req.params.userId;
    Post.findByIdAndUpdate({ _id: req.body.postId }, { $pull: { likes: like }, $inc: { totalLike: -1 } })
        .exec((err, result) => {
            if (err || !result) {
                return res.status(400).json({
                    error: errorHandler.getErrorMessage(err)
