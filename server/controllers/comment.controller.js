...
const likeComment = (req, res) => {
    ...
    Comment.findByIdAndUpdate({ _id: req.body.commentId }, { $push: { likes: like }, $inc: { totalLike: 1 } }, { new: true })
        .exec((err, result) => {
            if (err || !result) {
                return res.status(400).json({
                    error: errorHandler.getErrorMessage(err)
...
const unLikeComment = (req, res) => {
    var like = {};
    like.likeBy = req.params.userId;
    Comment.findByIdAndUpdate({ _id: req.body.commentId }, { $pull: { likes: like }, $inc: { totalLike: -1 } })
        .exec((err, result) => {
            if (err || !result) {
                return res.status(400).json({
                    error: errorHandler.getErrorMessage(err)
...
const checkLike = (req, res) => {
    ...
    Comment.find({ _id: commentId }, { likes: { $elemMatch: { likeBy: userId } } })
        .exec((err, result) => {
